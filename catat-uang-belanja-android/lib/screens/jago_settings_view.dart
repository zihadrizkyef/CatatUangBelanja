...
/// Pure Bank Jago settings layout: connect flow ("Hubungkan Gmail") or
/// connected state ("Putuskan"). No wallet picker — sync auto-creates and
/// manages one wallet per Kantong (pocket) as they're discovered. All
/// data/callbacks come from [JagoSettingsScreen] — this widget never reads
/// a repository/service or [Navigator] itself.
class JagoSettingsView extends StatelessWidget {
  ...
  List<Widget> _connectContent(AppPalette palette) {
    return [
      Text(
        'Hubungkan Gmail yang menerima notifikasi transaksi Bank Jago kamu — Kantong-Kantong Jago kamu (Kantong Terhubung, Tabungan, dst) otomatis muncul sebagai dompet baru di tab Dompet, dan tiap transaksi baru langsung tercatat di sana, Bun.',
  ...
  List<Widget> _connectedContent(AppPalette palette) {
    return [
      ...
      Text(
        'Kantong-Kantong Jago kamu muncul sebagai dompet di tab Dompet, dan tiap transaksi baru otomatis tercatat di sana setiap sinkron, Bun.',
        style: AppTheme.body(fontSize: 13, color: palette.textSecondary),
      ),
      const SizedBox(height: 18),
      SizedBox(
...
Widget previewJagoSettingsViewConnected() {
  return JagoSettingsView(
    loading: false,
    busy: false,
    status: JagoStatus(connected: true, connectedAt: DateTime(2026, 8, 1)),
    onConnect: null,
    onDisconnect: () {},
    onTapBack: () {},
  );
