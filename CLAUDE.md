...
- **Models** (`lib/models/`): plain Dart classes with `toMap`/`fromMap` for SQLite (de)serialization — no code generation. `IconType` (system/emoji/photo) is shared across wallets and categories per doc 2.4.
- **Screens** (`lib/screens/`): one file per bottom-nav tab (Beranda/Dompet/Rangkuman/Pengaturan), hosted by `app_shell.dart` which owns the `IndexedStack` + bottom nav + the "+" FAB that opens `TransactionSheet`. Screen names and UI strings are in Indonesian, matching the target audience — keep new UI copy in the same warm, informal tone described in doc section 2.6 (e.g. "Yuk catat belanja hari ini, Bun!"), not technical/corporate phrasing.
- **Theming**: `lib/theme/app_colors.dart` and `app_theme.dart` centralize the pastel palette and text styles (`AppTheme.body(...)`) — use these rather than ad-hoc `TextStyle`/`Color` values so light/dark and future background theming (doc 2.5) stay consistent.
