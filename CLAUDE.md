...
- **Icons**: `lib/theme/app_icons.dart` maps category/wallet `iconValue` keys and one-off UI icons (nav bar, sync status, medals, etc.) to bundled OpenMoji SVGs (`assets/icons/openmoji/`, rendered via `OpenMojiIcon`/`flutter_svg`) — chosen over raw emoji glyphs so icons render identically (and in full color) across every device/OS instead of depending on the platform's emoji font. OpenMoji is CC BY-SA 4.0; attribution lives in `assets/icons/openmoji/NOTICE.md` and is surfaced in-app under Pengaturan → Tentang Aplikasi.
